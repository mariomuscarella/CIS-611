...
-- =============================================
CREATE PROCEDURE [dbo].[SP_Report_NEW_Budget]

AS
BEGIN

	IF NOT EXISTS (SELECT* FROM sys.objects where
	object_id=object_id(N'[dbo].[NEW_Dept_Budget]') AND TYPE IN (N'U'))
   ...
   set @is_EMPTY = (select count(*) from VDept_Budget); 
	...
	insert into NEW_Dept_Budget (Dept_No,Dept_Name,COUNT_Emp,New_SUM_Salary,New_Avg_Salary) values (@dept_ID,@dept_NAME,@no_of_EMP,@sum_SALARY,@avg_SALARY);  
